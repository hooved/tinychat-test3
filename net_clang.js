...
var transformer = async function(state_dict) {
  ...
  modules.map((mod, i) =>
    mod.default({})  // pass an empty config here
      .then(instance => {
        // Now we have an explicit instance reference.
        // Set up the persistent directory:
        if (!instance.FS.analyzePath('/persistent').exists) {
          instance.FS.mkdir('/persistent');
        }
        // Mount IDBFS on that directory:
        //instance.FS.mount(instance.IDBFS, {}, '/persistent');
        ...
        return new Promise((resolve, reject) => {
          instance.FS.syncfs(true, (err) => {
            if (err) {
              ...
              // Call an exported function (if desired)
              //instance.ccall('start', 'void', [], []);
              resolve(instance);
            }
          });
        });
  ...
  //const asdf = wasm[0]._start();
  //console.log(wasm[0].FS.readdir('/persistent'));
