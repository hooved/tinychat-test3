  ...
  for (const [i, name] of weightNames.entries()) {
    for (const [j, buf_set] of buf_map_sets.entries()) {
      if (buf_set.has(i)) {
        ...
        wasm[j].HEAPU8.fill(1, bufPtr, state_dict[name].size);
      }
    }
  }
  ...
  return {
    run: (input0,start_pos) => {
      ...
      //for (let i = 0; i<wasm.length; i++) {
        //console.log(`wasm ${i}:`);
        //console.log(wasm[i].HEAPU8.length);
      //}

      //let wasm_in = wasm[0]._malloc(4);
      //let wasm_out = wasm[0]._malloc(8192);
      ...
      // transformer blocks, except last one
      for (let i=1; i<17; i++) {
        //wasm_in = wasm[i]._malloc(8192);
        wasm_in = ptrs[i][0];
        wasm[i].HEAPU8.set(wasm[i-1].HEAPU8.slice(prev_out, prev_out + 8192), wasm_in);
        //wasm[i-1]._free(prev_out);
        //wasm_out = wasm[i]._malloc(8192);
        wasm_out = ptrs[i][1];
        _net_out = wasm[i]._net(wasm_out, wasm_in, start_pos); // outputs a float* from last transformer block
        //wasm[i]._free(wasm_in);
        prev_out = wasm_out;
      }

      // output

      //const buf_352 = wasm[0]._malloc(8192);
      const buf_352 = ptrs[0][1];
      //const buf_5 = wasm[0]._malloc(4);
